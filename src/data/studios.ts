export type Studio = {
  id: string;
  name: string;
  minBudget: BudgetLevel; // minimum budget they accept
  size: 'small' | 'medium' | 'large';
  genres: string[]; // e.g., ['action', 'fantasy', 'romance']
  themes: string[]; // e.g., ['coming-of-age', 'war', 'music']
  ageGroups: string[]; // e.g., ['shonen', 'seinen', 'shojo']
  lengthPreference: 'short' | 'medium' | 'long'; // preferred episode count
  complexity: 'low' | 'medium' | 'high'; // story complexity they handle well
  pacing: 'slow' | 'medium' | 'fast'; // preferred pacing
  styleDescription: string; // short description affecting critic review
};
