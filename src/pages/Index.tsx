...
const formatArray = (values: string[]) => values.join(", ");
...
const getStudioFitScore = (manga: Manga, studio: Studio) => {
  ...
  if (studio.complexity !== "any") {
    if (
      complexityMap[studio.complexity] === complexityMap[getMangaComplexity(manga)]
    ) {
      score += 0.75;
    } else {
      score -= 0.25;
  ...
  if (studio.lengthPreference !== "any") {
    if (
      lengthMap[studio.lengthPreference] === lengthMap[getMangaLength(manga)]
    ) {
      score += 0.5;
    } else {
      score -= 0.25;
  ...
  if (studio.pacing !== "any") {
    if (
      pacingMap[studio.pacing] === pacingMap[getPacingPreference(manga)]
    ) {
      score += 0.5;
    } else {
      score -= 0.25;
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <Card className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Pick a fictional manga, assign a budget, choose a real studio, then
              wait for the AI critic review after the simulated airing run.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setSelectedManga(mangaList.find((manga) => manga.id === value) ?? null)
                }
              >
                ...
                <SelectContent>
                  {mangaList.map((manga) => (
                    <SelectItem key={manga.id} value={manga.id}>
                      {manga.title} — {formatStatus(manga)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setSelectedBudget(value as BudgetLevel)}
              >
                <SelectTrigger className="w-full">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setSelectedStudio(studios.find((studio) => studio.id === value) ?? null)
                }
              >
                <SelectTrigger className="w-full">
                ...
                <SelectContent>
                  {studios.map((studio) => (
                    <SelectItem key={studio.id} value={studio.id}>
                      {studio.name} — minimum {formatBudget(studio.minBudget)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
