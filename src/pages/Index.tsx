...
const inferGenres = (manga: Manga) => {
  ...
  if (title.includes("samurai") || title.includes("ninja") || title.includes("crusade") || title.includes("battalion") || title.includes("ronin") || title.includes("knight") || title.includes("errant") || title.includes("exorcist") || title.includes("monster") || title.includes("battle") || title.includes("dragon")) {
    genres.push("action");
  }
  if (title.includes("neon") || title.includes("cyber") || title.includes("mecha") || title.includes("drift") || title.includes("sky") || title.includes("harbor")) {
    genres.push("sci-fi");
  }
  if (title.includes("princess") || title.includes("kingdom") || title.includes("dragon") || title.includes("academy") || title.includes("grandmaster") || title.includes("familiar")) {
    genres.push("fantasy");
  }
  if (title.includes("café") || title.includes("cafe") || title.includes("winds") || title.includes("detective") || title.includes("gourmet") || title.includes("bento") || title.includes("convenience") || title.includes("lunch") || title.includes("karaoke") || title.includes("idol")) {
    genres.push("slice of life");
  }
  if (title.includes("ghost") || title.includes("spirit") || title.includes("spirits") || title.includes("exorcist") || title.includes("demon") || title.includes("monster") || title.includes("shrine")) {
    genres.push("supernatural");
  }
  if (title.includes("detective") || title.includes("mailbox") || title.includes("underpass")) {
    genres.push("mystery");
  }
  if (title.includes("cherry") || title.includes("postcards") || title.includes("roommate") || title.includes("princess") || title.includes("idol")) {
    genres.push("romance");
  }
  if (title.includes("mini golf") || title.includes("bicycle") || title.includes("drift") || title.includes("lunch")) {
    genres.push("sports");
  }
...
const inferThemes = (manga: Manga) => {
  ...
  if (title.includes("crusade") || title.includes("kingdom") || title.includes("dragon") || title.includes("ronin") || title.includes("battalion") || title.includes("knight") || title.includes("errant") || title.includes("bicycle") || title.includes("harbor")) {
    themes.push("adventure");
  }
  if (title.includes("samurai") || title.includes("ninja") || title.includes("mecha") || title.includes("battalion") || title.includes("exorcist") || title.includes("battle") || title.includes("drift")) {
    themes.push("action");
  }
  if (title.includes("café") || title.includes("cafe") || title.includes("winds") || title.includes("gourmet") || title.includes("bento") || title.includes("lunch") || title.includes("convenience") || title.includes("mailbox") || title.includes("cherry") || title.includes("postcards")) {
    themes.push("healing");
  }
  if (title.includes("cyber") || title.includes("neon") || title.includes("mecha") || title.includes("drift") || title.includes("sky")) {
    themes.push("sci-fi");
  }
  if (title.includes("princess") || title.includes("cherry") || title.includes("postcards") || title.includes("idol") || title.includes("roommate") || title.includes("mailbox")) {
    themes.push("romance");
  }
  if (title.includes("detective") || title.includes("mailbox") || title.includes("underpass") || title.includes("moonlit")) {
    themes.push("mystery");
  }
  if (title.includes("monster") || title.includes("karaoke") || title.includes("mini golf") || title.includes("salaryman") || title.includes("lunch")) {
    themes.push("comedy");
  }
  if (title.includes("mini golf") || title.includes("bicycle") || title.includes("drift") || title.includes("lunch")) {
    themes.push("sports");
...
const generateCriticReview = (manga: Manga, studio: Studio, budget: BudgetLevel) => {
  ...
  if (budgetNum < studioMinBudgetNum) {
    return [
      ...
      "",
      `${studio.name} declined the project. Their minimum accepted budget is ${formatBudget(studio.minBudget)}, but the assigned budget was only ${formatBudget(budget)}.`
    ].join("\n");
  }
  ...
  if (manga.chapters > 200) { score -= 1.5; } else if (manga.chapters < 60) { score += 0.5; } else { score += 0.25; }
  ...
  const ratingMap: Record<number, string> = {
    0: "Abysmal", 1: "Terrible", 2: "Poor", 3: "Below Average", 4: "Average",
    5: "Decent", 6: "Good", 7: "Very Good", 8: "Great", 9: "Excellent", 10: "Masterpiece"
  };
  const rating = ratingMap[finalScore];

  const budgetComment = budgetNum >= studioMinBudgetNum
...
const Index = () => {
  ...
  const handleProduce = () => {
    ...
    window.setTimeout(() => {
      setResult(generateCriticReview(producedManga, producedStudio, producedBudget));
